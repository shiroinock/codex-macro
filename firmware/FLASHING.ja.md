...
C100 Companion は、**Keychron C100 8K（通常動作時の VID `3434` / PID `042c`）にこのリポジトリの専用ファームウェアを書き込んで使います**。他の C100 や別機種のイメージとして流用しないでください。
...
Keychron Launcher を閉じます。既存の C100 デーモンが動いている場合は停止します。メニューバーの **C100 → デーモンを停止** を使います。CLI では次のコマンドで停止できます：
...
`layout=10x10`、`input=suppressed`、`keyboard_output=true` を確認します。`keyboard_output=false` は必要なキーボード出力機能がないことを示します。新しいイメージを書いたはずなら、イメージと対象を再確認してください。
...
- **handshake が失敗する**：このリポジトリの専用ファームウェアを使っているか確認します。
